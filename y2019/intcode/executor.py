...
async def run(prog: Program, reader: Reader = stdin, writer: Writer = stdout) -> int:
    ...
    while True:
        ...
        instr = opcode()
        if instr is Opcode.Add:
            write(3, read(1) + read(2))
            ptr += 4
        elif instr is Opcode.Multiply:
            write(3, read(1) * read(2))
            ptr += 4
        elif instr is Opcode.Input:
            ...
            ptr += 2
        elif instr is Opcode.Output:
            # Write is async because it may need to acquire a lock
            await writer(read(1))
            ptr += 2
        elif instr is Opcode.JmpIfTrue:
            if read(1):
                ptr = read(2)
            else:
                ptr += 3
        elif instr is Opcode.JmpIfFalse:
            if not read(1):
                ptr = read(2)
            else:
                ptr += 3
        elif instr is Opcode.LessThan:
            val = 0
            if read(1) < read(2):
                val = 1
            write(3, val)
            ptr += 4
        elif instr is Opcode.Equals:
            val = 0
            if read(1) == read(2):
                val = 1
            write(3, val)
            ptr += 4
        elif instr is Opcode.SetRelBase:
            rel_base += read(1)
            ptr += 2
        elif instr is Opcode.Halt:
            return ptr
        else:
