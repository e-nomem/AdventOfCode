...
def brute_force(x_range, y_range):
    sim_launches = (launch((dx, dy), x_range, y_range) for dx in range(x_range[1] + 1) for dy in range(y_range[0], 100))
    return sum(1 for max_y in sim_launches if max_y is not None)
