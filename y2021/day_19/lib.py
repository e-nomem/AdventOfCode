...
class Scanner:
    ...
    def map_position(self, other_scanner: "Scanner", min_intersections: int) -> bool:
        ...
        this_beacon_a, other_beacon_a, beacon_intersections = scanner_intersection

        # We only really need one of the intersections TBH, but some intersections are unsuitable
        for this_beacon_b_id, other_beacon_b_id in beacon_intersections:
            ...
            this_beacon_b = next(b for b in self.beacons.values() if b.id == this_beacon_b_id)
            other_beacon_b = next(b for b in other_scanner.beacons.values() if b.id == other_beacon_b_id)

            # Take the matched beacons and shift them into a position relative to the other end of the match
            pos_a = diff_position(this_beacon_a.position, this_beacon_b.position)
            pos_b = diff_position(other_beacon_a.position, other_beacon_b.position)
            ...
            other_scanner.position = tuple(
                map(
                    ...
                    zip(this_beacon_a.position, self.position, other_beacon_a.position),
                ),
            )
        ...
        print(f" -> Scanner {self.id}: {this_beacon_a.position}")
        print(f" -> Scanner {other_scanner.id}: {other_beacon_a.position}")
        return False
